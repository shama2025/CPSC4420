...
long int run_philosopher(Philosopher *philosopher)
{
	...
	while (philosopher->food_consumed < HUNGER) // Loop until philosopher has eaten enough
	{
		if (philosopher->id % 2 == 0) // If philosopher has an even ID number
		{
			get_chopstick(philosopher->left_chopstick); // Even philosophers pick up left chopstick first
			get_chopstick(philosopher->right_chopstick); // Then pick up right chopstick
		}
		else // If philosopher has an odd ID number
		{
			get_chopstick(philosopher->right_chopstick); // Odd philosophers pick up right chopstick first
			get_chopstick(philosopher->left_chopstick); // Then pick up left chopstick
		}
		eat(philosopher); // Philosopher eats using both chopsticks
		return_chopstick(philosopher->right_chopstick); // Return right chopstick when done eating
		return_chopstick(philosopher->left_chopstick); // Return left chopstick when done eating
	}
	return philosopher->food_consumed; // Return the amount of food the philosopher consumed
}
