...
Thread* schedule_lottery(Thread* threads) {
    ...
    if (eligible_weight_sum == 0) {  // Fixed variable name from total_tickets to eligible_weight_sum
        return NULL;
    }

    // Step 2: Select a random ticket
    ran = rand() % eligible_weight_sum;  // Fixed variable name
...
Thread* schedule_wfq(Thread* threads) {
    ...
    double max_service_deficit = -1.0; // Highest service deficit found so far (renamed from min_service_deficit)
    ...
    for (int i = 0; i < NUM_THREADS; i++) {
        if (threads[i].state == RUNNABLE) {
            ...
            if (index == -1 || service_deficit > max_service_deficit) { // Changed comparison direction
                max_service_deficit = service_deficit;
                index = i;
            }
        }
    }
